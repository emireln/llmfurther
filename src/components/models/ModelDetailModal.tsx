...
export const ModelDetailModal: React.FC<ModelDetailModalProps> = ({ model, onClose }) => {
  const { t } = useLanguage();
  const [activeTab, setActiveTab] = useState<'quickstart' | 'specs' | 'provider'>('quickstart');
  ...
  return (
    ...
    >
      <div
        ...
        className="relative w-full max-w-2xl max-h-[90vh] flex flex-col rounded-xl border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 shadow-2xl overflow-hidden"
      >
        {/* Modal Header */}
        <div className="flex items-start justify-between p-5 border-b border-neutral-200 dark:border-neutral-800">
          <div className="flex items-start gap-3.5">
            <ModelLogo
              ...
              size={36}
              className="mt-0.5 rounded-sm p-1 border border-neutral-200 dark:border-neutral-800 bg-neutral-50 dark:bg-neutral-950/60"
            />
            <div>
        ...
        <div className="bg-accent/5 dark:bg-accent/10 border-b border-accent/20 px-5 py-3 flex items-center gap-2.5">
          <Zap className="w-4 h-4 text-accent shrink-0" />
          <div className="text-xs text-neutral-800 dark:text-neutral-200 font-sans">
            <span className="font-bold text-accent font-mono uppercase tracking-wider text-[11px] mr-1.5">
              {t.card.quota}:
            </span>
        ...
        {/* Modal Tabs */}
        <div className="flex border-b border-neutral-200 dark:border-neutral-800 px-5 bg-neutral-50 dark:bg-neutral-950/50 text-xs font-mono">
          <button
          ...
          {activeTab === 'specs' && (
            <div className="space-y-4 text-xs font-mono">
              <div className="grid grid-cols-2 gap-3">
                <div className="p-3 rounded-lg border border-neutral-200 dark:border-neutral-800 bg-neutral-50 dark:bg-neutral-950/40">
                  <span className="text-neutral-500 block text-[11px] mb-1">Max Context</span>
                ...
                <div className="p-3 rounded-lg border border-neutral-200 dark:border-neutral-800 bg-neutral-50 dark:bg-neutral-950/40">
                  <span className="text-neutral-500 block text-[11px] mb-1">SWE-bench Rating</span>
                  <span className="text-sm font-bold text-neutral-900 dark:text-neutral-100">
                    {model.sweScore || 'Unranked / General LLM'}
                  </span>
                </div>
              </div>

              <div>
                <span className="text-neutral-500 block text-[11px] mb-1.5">Modalities</span>
                <div className="flex flex-wrap gap-1.5">
                  {model.type?.map(m => (
                    <span
                      key={m}
                      className="px-2 py-0.5 rounded bg-neutral-100 dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 text-neutral-700 dark:text-neutral-300 uppercase"
                    >
                      {m}
                    </span>
                  ))}
              ...
              <div>
                <span className="text-neutral-500 block text-[11px] mb-1.5">Access Category</span>
                <div className="flex flex-wrap gap-1.5">
                  {model.access?.map(acc => (
                    <span
                      key={acc}
                      className="px-2 py-0.5 rounded bg-accent/10 border border-accent/30 text-accent font-semibold"
                    >
        ...
        <div className="p-4 border-t border-neutral-200 dark:border-neutral-800 bg-neutral-50 dark:bg-neutral-950/70 flex items-center justify-between gap-3">
          {model.url ? (
            <a
              href={model.url}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-accent text-white font-mono text-xs font-semibold hover:bg-accent-hover transition-colors shadow-sm"
            >
          ...
          <button
            onClick={onClose}
            className="px-4 py-2 rounded-lg border border-neutral-200 dark:border-neutral-700 text-neutral-700 dark:text-neutral-300 font-mono text-xs hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-colors"
          >
            {t.modal.close}
          </button>
        </div>
